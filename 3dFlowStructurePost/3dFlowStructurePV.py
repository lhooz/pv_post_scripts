...
for ki in kinematics_arr:
    ...
    geop_output_files = os.path.join(
        result_data_folder, 'geop_' + '{0:.2f}'.format(time) + '.csv')
    #----------------------------------------
    #time
    ...
    cp = FindSource('Cp')
    SetActiveSource(cp)
    cp.Function = 'p/' + '{0:.8f}'.format(Cp_constant)
    #-------------------------------------------
    renderView1.Update()
    renderView2.Update()
    renderView3.Update()
    ...
    # export view
    ExportView(geop_output_files, view=spreadSheetView1)
